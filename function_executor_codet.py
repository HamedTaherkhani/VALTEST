...
def check_correctness_with_test_cases(task_id, prompt, completion, test_cases, timeout, is_unittest=False):
    ...
    def unsafe_execute():
        ...
        with create_tempdir():
            ...
            os.chdir = chdir
    if is_unittest:
        res = run_unit_tests_parallel(code_str=completion,test_list=test_cases)
        return dict(
            task_id=task_id,
...
def check_correctness(task_id: str, prompt: str, completion: str, test: str, entry_point: str, timeout: float, is_unittest=False) -> Dict:
    """
