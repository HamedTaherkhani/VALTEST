...
class DataManager:
    def __init__(self, dual_exec_results, sampled_code_by_task, sampled_test_case_by_task, limit):
        ...
        ## added this
        self.valtest_scores = dict()
        self._set_valtest_scores()
    ...
    def _get_solution_frequency(self):
        ...
        print(f'len self.solution_frequency_by_task : {len(self.solution_frequency_by_task.keys())}')
    ...
    def _get_test_case_frequency(self):
        for task_id in self.sampled_test_case_by_task.keys():
            ...
            self.test_case_frequency_by_task[task_id] = Counter([a[0] for a in task_test_cases])
        print(f'len test_case_frequency_by_task : {len(self.test_case_frequency_by_task.keys())}')
    def _set_valtest_scores(self):
        valtest_scores = {}
        for key, result in self.sampled_test_case_by_task.items():
            tests = result[0]
            test_scores = {}
            for idx, test_case in enumerate(tests):
                test_scores[f'{idx}'] = test_case[1]
            valtest_scores[key] = test_scores
        # print(valtest_scores)
        print(f'len valtest_scores : {len(valtest_scores.keys())}')
        self.valtest_scores = valtest_scores
    ...
    def _get_passed_solution_test_case_pairs_by_task(self):
        ...
        print(f'len passed_unique_test_cases_by_task: {len(self.passed_unique_test_cases_by_task.keys())}')
    ...
    def _get_expanded_dual_exec_result(self):
        ...
        print(f'len self.expanded_passed_solution_test_case_pairs_by_task {len(self.expanded_passed_solution_test_case_pairs_by_task.keys())}')
...
class DualAgreement:
    ...
    def _get_solution_passed_case_set(self):
        print(f'len  self.dual_exec_results_by_task {len( self.dual_exec_results_by_task)}')
    ...
    def _get_caseset_passed_solutions(self):
        print(f'len self.solution_passed_cases_by_task.keys() {len(self.solution_passed_cases_by_task.keys())}')
    ...
    def get_sorted_solutions_without_iter(self, use_valtest_scores):
        ...
        print(f'len caseset : {len(self.caseset_passed_solutions_by_task.keys())}')
        for task_id in self.caseset_passed_solutions_by_task.keys():
            ...
            for case_set in self.caseset_passed_solutions_by_task[task_id].keys():
                ...
                if use_valtest_scores:
                    case_set_score = 0
                    for cc in case_set:
                        # print((task_id, cc))
                        case_set_score += self.data_manager.valtest_scores[task_id][f'{cc}']
                else:
